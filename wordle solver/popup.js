...
function getCounts(words) {
  ...
  for (let i = 0; i < words[0].length; i++) {
    allCounts[i] = new Dict();
    for (let w of words) {
      allCounts[i][w[i]] = allCounts[i].get(w[i]) + 1;
    }
  }
...
function updateWordScores(wordsWithScores) {
  wordDiv.textContent = wordsWithScores;
}


function getWord(words, allCounts) {
  let wordsWithScores = [];
  ...
  words.forEach(candidate => {
    ...
    wordsWithScores.push([candidate, score]);;
...
async function updateWordList(tab) {
  ...
  chrome.scripting.executeScript({
    ...
    if (feasibleWords.length == 0) {
      updateWordScores("No words left, did you win?");
    }
    else {
      counts = getCounts(feasibleWords);
      bestNextWord = getWord(feasibleWords, counts);
    }
  });
}
...
function getResults() {
  ...
  gameApp.querySelectorAll("game-row").forEach(row => {
    if (row.getAttribute("letters").length > 0) {
      res.push(Array.from(row.shadowRoot.querySelectorAll("game-tile")).map(t => [t.getAttribute("letter"), t.getAttribute("evaluation")]));
    }
  });
  return res;
}
